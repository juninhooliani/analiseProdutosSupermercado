...
# Função para corrigir possíveis erros de acentuação
def corrigir_acentuacao_dia_semana(dia):
    return dia.replace("Ã¡", "á").replace("Ã©", "é").replace("Ã³", "ó").replace("Ã£", "ã").replace("Ãª", "ê")
...
def identificar_categoria(descricao, falhas_consecutivas, max_falhas_alcancado):
    """
    Identifica a categoria usando ChatGPT ou categorização local.
    Para de tentar o ChatGPT após 3 falhas consecutivas.
    """
    if not max_falhas_alcancado:
        categoria = conversar_com_chatgpt(f"Qual é a categoria do produto: {descricao}?")
        if categoria:
            return categoria.strip(), 0, False  # Resposta válida, resetando contador de falhas
        else:
            falhas_consecutivas += 1
            if falhas_consecutivas >= 3:
                print("Máximo de falhas ao ChatGPT atingido. Usando apenas categorização local.")
                return identificar_categoria_local(descricao), falhas_consecutivas, True
    else:
        # Se o limite já foi atingido, usa apenas categorização local
        categoria = identificar_categoria_local(descricao)
    return categoria, falhas_consecutivas, max_falhas_alcancado
...
def processar_cupom(cupom_texto):
    ...
    falhas_consecutivas = 0  # Contador de falhas consecutivas ao ChatGPT
    max_falhas_alcancado = False  # Controle se o limite de falhas já foi atingido
    ...
    if endereco_idx is not None and endereco_idx >= 1:
        ...
        estabelecimento = "Estabelecimento Desconhecido"
    ...
    for linha in linhas:
        ...
        if match:
            ...
            # Identifica a categoria, limitando falhas consecutivas ao ChatGPT
            categoria, falhas_consecutivas, max_falhas_alcancado = identificar_categoria(
                descricao, falhas_consecutivas, max_falhas_alcancado
            )
...
def atualizar_planilha(dados, arquivo_excel):
    ...
    # Corrige a coluna 'Dia da Semana' após o processamento
    df_final['Dia da Semana'] = df_final['Dia da Semana'].apply(corrigir_acentuacao_dia_semana)
...
arquivo_excel = 'compras.xlsx'
atualizar_planilha(dados_cupons, arquivo_excel)
>>>>>>> 64af50b794c1bc540f96585a2770ae207302190a
